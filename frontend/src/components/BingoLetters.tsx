...
const BingoLetters: React.FC<BingoLettersProps> = ({
  ...
  letterClassName = 'text-center py-3 rounded-xl font-bold text-xl transition-all duration-300 relative border-2 backdrop-blur-sm'
}) => {
  return (
    <div className="w-full max-w-[360px] mx-auto">
      <div className="bg-gradient-to-br from-slate-800/60 to-slate-700/40 backdrop-blur-sm rounded-2xl p-2 border border-red-500/30 shadow-lg">
        ...
        <div className={containerClassName}>
          {BINGO_LETTERS.map((letter: string, idx: number) => {
            ...
            let letterStyle = letterClassName;
            letterStyle += ' ' + (active ? (marked ? 'text-red-400 bg-gradient-to-br from-red-500/30 to-red-600/20 border-red-500 shadow-[0_0_15px_#EF4444]' : 'text-red-400 border-red-500/50 bg-gradient-to-br from-red-500/10 to-red-600/5 hover:bg-gradient-to-br hover:from-red-500/20 hover:to-red-600/10 cursor-pointer hover:scale-105 hover:shadow-lg') : 'text-red-400/40 border-red-500/20 bg-gradient-to-br from-slate-700/30 to-slate-600/20');
            return (
              <div 
                key={letter} 
